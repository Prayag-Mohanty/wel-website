...
def inventory():
    ...
    types = [r[0] for r in db.session.query(Component.component_type).distinct().all() if r[0]]
...
def admin_dashboard():
    ...
    total_comp  = Component.query.count()
    total_teams = Team.query.count()
    low_stock   = Component.query.filter(Component.quantity <= 2).all()
    return render_template('admin/dashboard.html', pending=pending,
                           total_comp=total_comp, total_teams=total_teams,
                           low_stock=low_stock)
...
@admin_required
def admin_inventory():
    components = Component.query.order_by(Component.sr_no).all()
    return render_template('admin/inventory.html', components=components)
...
def update_quantity(cid):
    ...
    comp.quantity = int(request.form.get('quantity', comp.quantity))
    db.session.commit()
